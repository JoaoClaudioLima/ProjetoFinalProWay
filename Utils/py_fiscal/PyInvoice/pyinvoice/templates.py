...
from Utils.py_fiscal.PyInvoice.pyinvoice.components import SimpleTable, TableWithHeader, PaidStamp
from Utils.py_fiscal.PyInvoice.pyinvoice.models import PDFInfo, Item, Transaction, InvoiceInfo, ServiceProviderInfo, ClientInfo
...
class SimpleInvoice(SimpleDocTemplate):
    ...
    def _invoice_info_data(self):
        if isinstance(self.invoice_info, InvoiceInfo):
            props = [('invoice_id', 'NFe'), ('invoice_datetime', 'Data do recibo'),
    ...
    def _build_invoice_info(self):
        ...
        if invoice_info_data:
            self._story.append(Paragraph('Nota Fiscal', self._defined_styles.get('RightHeading1')))
    ...
    def _service_provider_data(self):
        if isinstance(self.service_provider_info, ServiceProviderInfo):
            props = [('name', 'Razão social'), ('street', 'Logradouro'), ('city', 'Cidade'), ('state', 'Estado'),
                     ('country', 'País'), ('post_code', 'CEP'), ('vat_tax_number', 'Vat/Tax number')]
    ...
    def _client_info_data(self):
        ...
        props = [('name', 'Nome'), ('street', 'Street'), ('city', 'City'), ('state', 'State'),
    ...
    def _build_service_provider_and_client_info(self):
        if isinstance(self.service_provider_info, ServiceProviderInfo) and isinstance(self.client_info, ClientInfo):
            ...
            table_data = [
                [
                    Paragraph('Livros para Todxs ', self._defined_styles.get('Heading1')), '',
                    '',
                    Paragraph('Cliente', self._defined_styles.get('Heading1')), ''
                ]
            ]
            table_style = [
                ('SPAN', (0, 0), (1, 0)),
    ...
    def _item_data_and_style(self):
        ...
        self._story.append(
            Paragraph('Detalhes dos produtos', self._defined_styles.get('Heading1'))
        )

        item_data_title = ('Nome do Produto', 'Unidades', 'Preço por unidade', 'Preço (somado)')
    ...
    def _transactions_data(self):
        ...
        if transaction_table_data:
            transaction_table_data.insert(0, ('Id da Transação', 'Método de pagamento', 'Data da transação'))
    ...
    def _build_transactions(self):
        ...
        if transaction_table_data:
            self._story.append(Paragraph('Detalhes da compra', self._defined_styles.get('Heading1')))
